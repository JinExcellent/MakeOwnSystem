...
void put_fonts8_asc(unsigned char *vram, int xsize, int x, int y, char c, char *s) {
  ...
  for (; *s != '\0'; s++) {
    //这里左移四位的原因是Hankaku有4096个地址空间，为了能够全部访问才左移四位。移位后共12位，正好可以表示4096个地址空间
...
//len:字符长度 b:背景颜色
void put_font8_asc_sht(struct Sheet *sht, int x, int y, int c, int b, char *s, int len){
    box_fill8(sht->buf, sht->bxsize, b, x, y, x + 8 * len - 1, y + 15);
