        ...
        if(fifo32_status(&task->fifo) == 0){
            ...
            if(256 <= data && data <= 511){
                if(data == 8 + 256){     //退格键码值为8
                    if(cursor_x > 16){
                        put_font8_asc_sht(sheet, cursor_x - 8, 28, COL8_FFFFFF, COL8_000000, " ", 2);
                        cursor_x -= 8;
                    }
                }else{
                    //一般字符
                    if(cursor_c < 240){
                        s[0] = data - 256;
                        s[1] = 0;
                        put_font8_asc_sht(sheet, cursor_x, 28, COL8_FFFFFF, COL8_000000, s, 1);
                        cursor_x += 8;
                    }
                }
            }

            box_fill8(sheet->buf, sheet->bxsize, cursor_c, cursor_x, 28, cursor_x + 1, 43);     //绘制光标
            sheet_refresh(sheet, cursor_x, 28, cursor_x + 1, 44);
        }
    }
}
...
int main(void) {
    ...
    task_cons->tss.esp  = memman_alloc_4K(memman, 64 * 1024) + 64 * 1024 - 8;  //分配栈空间
            ...
            if(256 <= key && key <= 511){   //键盘接收数据范围 
                ...
                if('A' <= s[0] && s[0] <= 'Z'){
                    if(((key_leds & 4) == 0 && key_shift == 0) || ((key_leds & 4) != 0 && key_shift != 0)){
                        ...
                        sprintf(s, "%s", s);
                        put_font8_asc_sht(sht_back,0, 200, COL8_FFFFFF, COL8_008484, s, sizeof(s));
