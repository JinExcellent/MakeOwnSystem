    ...
    while(1){
        ...
        if(fifo32_status(&task->fifo) == 0){
            ...
            if(data == 2)
                cons.cursor_c = COL8_FFFFFF;
            if(data == 3){
                box_fill8(sheet->buf, sheet->bxsize, COL8_000000, cons.cursor_x, cons.cursor_y, cons.cursor_x + 1, cons.cursor_y + 15);
                cons.cursor_c = -1;
            }
            //键盘数据
            if(256 <= data && data <= 511){
                ...
                if(cons.cursor_c >= 0)
                    box_fill8(sheet->buf, sheet->bxsize, cons.cursor_c, cons.cursor_x, cons.cursor_y, cons.cursor_x + 1, cons.cursor_y + 15);     //绘制光标
...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline){
    ...
    if(finfo){
        ...
        *((int *)0xfe8) = (int) p;      //向该地址传入程序在内存中的首地址
