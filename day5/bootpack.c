...
//程序主入口地址
int main(void) {
    struct BootInfo *binfo = (struct BootInfo *)0x0ff0;     //启动信息首地址位0x0ff0
...
void put_fonts8_asc(unsigned char *vram, int xsize, int x, int y, char c, char *s) {
  ...
  for(; *s != '\0'; s++) {
    ...
    x += 8;     //每个字符宽度为8位，在每次打印完一个字符后，显存地址向右偏移
  }
}
...
void init_gdtidt(void) {
    //分别设置GDT和IDT表的首地址
  struct SegmentDescriptor *gdt = (struct SegmentDescriptor *)0x00270000;   //地址范围 0x00270000~0x0027ffff
  struct GateDescriptor *idt = (struct GateDescriptor *)0x0026f800;

  for (int i = 0; i < 8192; i++) {      //段寄存器为16位，后三位不能使用，共有8192个段，每段8字节，共64kB
    set_segmdesc(gdt + i, 0, 0, 0);     //指针加法会根据指针指向的数据类型自动调整步长
  }
